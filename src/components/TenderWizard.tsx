                    ...
                    <div className="px-5">
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                            ...
                            <EquipeEtPieces
                                ...
                                missingSpecialties={missingSpecialties}
                                requiredSpecialtyIds={formData.required_specialty_ids}
                                refSpecialties={refSpecialties}
                                allCoveredSpecialtyIds={allCoveredSpecialtyIds}
                                requiredSkills={formData.required_skills}
                                typeGroupement={formData.type_groupement}
                                potentialGain={potentialGain}
                                carouselIndex={carouselIndex}
                                isOwner={isOwner}
                                isLocked={isLocked}
                                ...
                                onOuvrirMembre={setSelectedMemberIndex}
                                onOuvrirCoordination={() => setShowDocDetails(true)}
                                onOuvrirCompetences={() => setShowSkillsModal(true)}
                                onAjouterMembre={() => setShowAddManualModal(true)}
                                onOuvrirReseau={() => setShowCollabPicker(true)}
                                onRetirerMembre={setShowRemoveConfirm}
                                onRelancerInvitation={handleResendInvitation}
                                onRetirerCompetence={removeComp}
                            />
    ...
    return (
        <div className="w-full p-4 mx-auto h-full flex flex-col gap-6">
            <div className={`flex-1 ${GLASS_MODAL_STYLE} flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 w-full mx-auto h-full relative`}>
                ...
                />
                <DCEPiecesModal
                    ouvert={showDCEPiecesModal}
                    documents={formData.dce_documents || []}
                    isOwner={isOwner}
                    isLocked={isLocked}
                    isUploading={isUploadingDCE}
                    zipEnCours={zipDCEEnCours}
                    onFermer={() => setShowDCEPiecesModal(false)}
                    onDeposer={handleDCEFileUpload}
                    onRemplacer={handleDCEFileReplace}
                    onSupprimer={handleDCEFileDelete}
                    onMajPiece={majPieceDCE}
